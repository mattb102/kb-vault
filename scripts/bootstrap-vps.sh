...
# ── 8. cron — promote_patterns + nightly identity rebuild ───────────
# Both only install if ANTHROPIC_API_KEY is present in .env.
if grep -q "^ANTHROPIC_API_KEY=sk-" "$REPO/.env" 2>/dev/null; then
  ...
  (
    crontab -l 2>/dev/null | grep -v "promote-patterns" | grep -v "cron-rebuild-identity"
    echo "$PROMOTE_CRON"
    echo "$IDENTITY_CRON"
  ) | crontab -
  ok "Crons installed: promote_patterns (Sun 4am) + identity rebuild (nightly 4:05am)"
else
