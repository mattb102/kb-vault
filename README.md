...
1. **A folder of markdown notes** (your "vault") that you can browse in
   [Obsidian](https://obsidian.md) — a free app that turns a folder of `.md`
   files into a beautiful, searchable notebook. Your notes are just files on
   your computer; Obsidian is how you see them.

2. **An MCP server** — a small program that runs in the background and gives
   Claude the ability to *read and write* those notes. MCP (Model Context
   Protocol) is how Claude Code connects to tools outside itself; this server
   is what lets Claude search your vault, log metrics, write observations, and
   remember things across sessions. Without it, Claude has no idea your vault
   exists.

Together: you talk to Claude, Claude talks to your vault, you browse the result
in Obsidian. That's the whole thing.

## Setting it up

**You don't follow a manual — Claude does.** Open this folder in Claude Code
and type:
...
"authorize" button), checks each step worked, and remembers where you are if
you stop and come back. Plan for about an hour. You need no programming
knowledge — Windows, Mac, and Linux are all fine.
